...
def parse_address(root, xmlns_url):
	...
	parsed_index, parsed_address, unparsed_index = ([] for i in range(3))
	for unit_index, unit in enumerate(root.iter(f'{{{xmlns_url}}}serviceUnit')):
		...
		if detail and not weak_large:
			unparsed_index.append(unit_index)
			# do not trim, address is unparsed
			tcname = unit.find('xmlns:nameTChinese', xmlns).text
			address = unit.find('xmlns:addressEnglish', xmlns).text
			address = re.sub(r', ?,', ',', re.sub(bracket_regex, '', tcname) + ', ' + address)
	...
	print(f'Parsing ratio is {len(parsed_address)}:{len(unparsed_index)}')
	# return all required data for traversing XML
	return [parsed_index, parsed_address, unparsed_index]
...
print('Cleaning up work environment')
with contextlib.suppress(FileNotFoundError):
	os.remove('unitinfo.js')
print('All data generated by this script is contained in scripts/unitinfo.js')
print('XML request sent')
xmlns_url = 'http://elderlyinfo.swd.gov.hk'
service_root = request_service_xml('http://elderlyinfo.swd.gov.hk/sites/ltc-swd/files/rche_rsp_list.xml')
