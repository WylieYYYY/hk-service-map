...
def get_override_table(xml_url, batch_size, script_path):
	...
	if not os.path.exists(f'{script_path}/override.csv'):
		...
		# UTF8 BOM for Excel
		with open(f'{script_path}/override.csv', 'w', encoding='utf-8-sig') as f:
		...
		with open(f'{script_path}/override.csv', 'r', encoding='utf-8-sig') as f:
			for line in f:
				fields = line.split('\t')
...
def batch_req(parsed_index, parsed_address, root, xmlns_url, ratio, batch_size, override_table, script_path):
	...
	with open(f'{script_path}/scripts/unitinfo.js', 'w', encoding='utf-8') as f:
		f.write('var longlat = ')
...
batch_size = 50
service_url, batch_size, override_table = get_override_table(service_url, batch_size, script_path)
...
batch_req(result_list[0], result_list[1], service_root, xmlns_url, result_list[2], batch_size, override_table, script_path)
